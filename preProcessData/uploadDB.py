...
def create_db():
    # 连接数据库服务器-*-*-
    connect = pymysql.connect(user="root",password="Intel@123", host="127.0.0.1", 
                              port=3306, db="KB_QA",charset="utf8")
    ...
    sql = """create table nlpQA(
        entity VARCHAR(50) character set utf8 collate utf8_unicode_ci,
        attribute VARCHAR(50) character set utf8 collate utf8_unicode_ci, 
        answer VARCHAR(255) character set utf8 
        collate utf8_unicode_ci)"""  # ()中的参数可以自行设置
    conn.execute("drop table IF EXISTS nlpQA")  # 如果表存在则删除
...
def loaddata():
    ...
    # 读取本地CSV文件
    df = pd.read_csv("./DBData/clean_triple.csv", sep=',', encoding='utf-8')
    print(df)
    ...
    pd.io.sql.to_sql(df, 'nlpQA', con=engine, index=False, if_exists='append', chunksize=10000)
...
if __name__ == '__main__':
    create_db()
    loaddata()
    # sql = "select * from nlpQA where entity = '高等数学'"
