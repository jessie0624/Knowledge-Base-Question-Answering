...
from BertCRFModel import BertCRF 
from transformers import AdamW, WarmupLinearSchedule

from sklearn.metrics import classification_report

logger = logging.getLogger(__name__)
#Normally   CRFlabels includes:["O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X", "[CLS]", "[SEP]"]
# 在这个项目中只需要识别三个类型的项目即可  LABELS = ["O", "B-LOC", "I-LOC"] ，

CRF_LABELS = ["O", "B-LOC", "I-LOC"]

def statical_real_sentences(input_ids: torch.Tensor,
                            mask:torch.Tensor,
                            predict: list) -> list:
    ...
    new_mask = mask[:, 2:]
    real_ids = []
    for i in range(new_ids.shape[0]): ## batch_size
        seq_len = new_mask[i].sum()## 1 是有词, 0是padding
...
def CRF_convert_examples_to_features(examples, tokenizer, max_length=512,
                                    label_list = None, pad_token=0,
    ...
    for (ex_index, example) in enumerate(examples):
        inputs = tokenizer.encode_plus(example.text, add_special_tokens=True,
                                    max_length=max_length, truncate_first_sequence=True)
        ...
        ## 第一个[0] 是 [cls], 第二个[0] 是[sep],后面的是padding
        labels_ids = [0] + [label_map[l] for l in examples.label] + [0] + [0] * padding_length
...
class NERProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        return self._create_exaCRFInputExamplemples(
        ...
        with codecs.open(path, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(tokens) == 2:
                    ...
                    label_list = []
        examples = []
        for i, (sent, lable) in enumerate(lines):
            examples.append(CRFInputExample(guid=i, text=' '.join(sent), lable=lable))
        return examples
...
def load_and_cache_example(args, tokenizer, processor, data_type):
    ...
    if data_type not in type_list:
        raise ValueError('data_type must be one of {}'.foarmt(" ".join(type_list)))
    ...
    if os.path.exists(cached_features_file):
        ...
        features = CRF_convert_examples_to_features(examples=examples,
                                                    tokenizer=tokenizer,
...
def trains(args, train_dataset, eval_dataset, model):
    ...
    no_decay = ['bias', 'LayerNorm.weigth', 'transitions']
    ...
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), decs='Epoch')
    set_seed(args)
    best_f1 = 0.
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step+1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % 100 == 0:
                    best_f1 = evaluate_and_save_model(args, model, eval_dataset, global_step, best_f1)
...
def evaluate(args, model, eval_dataset):
    ...
    for batch in tqdm(eval_dataloader, desc='Evaluating'):
        ...
        with torch.no_grad():
            ...
            real_token_label.extend(statical_real_sentences(batch[3], batch[1], temp_pred))
    loss = np.array(loss).mean()
    real_token_label = np.array(flatten(real_token_label))
    pred_token_label = np.array(flatten(pred_token_label))
    assert real_token_label.shape == pred_token_label
    ret = classification_report(y_true=real_token_label, y_pred=pred_token_label,
                                target_names=CRF_LABELS, output_dict=True)
    model.train()
    return ret


def evaluate_and_save_model(args, model, eval_dataset, global_steps, best_f1):
    ret = evaluate(args, model, eval_dataset)

    precision_b = ret['B-LOC']['precision']
    recall_b = ret['B-LOC']['recall']
    f1_b = ret['B-LOC']['f1-score']
    support_b = ret['B-LOC']['support']

    precision_i = ret['I-LOC']['precision']
    recall_i = ret['I-LOC']['recall']
    f1_i = ret['I-LOC']['f1-score']
    support_i = ret['I-LOC']['support']
...
def main():
    ...
    tokenizer_inputs = ()
    tokenizer_kwards = {'do_lower_case':False, "max_len":args.max_seq_length, "vocab_file":args.vocab_file}
    tokenizer = BertTokenizer(*tokenizer_inputs, **tokenizer_kwards)
    model = model.to(args.device)
    train_dataset = load_and_cache_example(args, tokenizer, processor, 'train')
    eval_dataset = load_and_cache_example(args, tokenizer, 'dev')
    test_dataset = load_and_cache_example(args, tokenizer, 'test')
